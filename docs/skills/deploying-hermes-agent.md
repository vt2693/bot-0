...
### Text-to-Speech (TTS)
Confidence: 100% — Toggleable per-user from Voice & Minutes menu (`ac:tts_toggle`). State stored in MemoryStore as `tts_enabled=true|false` per chat_id. TTS model switchable from sub-menu (`mn:tts_model` → `ac:tts_model:*`), stored as `tts_model=<name>` per chat_id with default `edge-tts/en-US-AndrewMultilingualNeural`. Menu shows ✅ active and ⭐ default indicators. **Auto-Play toggle** (`ac:tts_autoplay_toggle`) sends TTS as a video note (round, auto-plays) instead of voice message — stored as `tts_autoplay=true|false` per chat_id (default `false`). On each text response, background task: `tg_tts.synthesize()` → router-0 TTS (MP3) → either `_send_voice_direct()` via `tg_tts.to_opus()` (voice message, tap to play) or `_send_video_note_direct()` via `tg_tts.to_video_note()` (video note, auto-plays). 3 retries with backoff. Input truncated to 4000 chars. Defaults off.
